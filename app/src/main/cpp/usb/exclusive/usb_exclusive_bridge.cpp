...
constexpr int kEventLoopWaitTimeoutMs = 5;
...
// 同步模式（无反馈端点）时钟漂移补偿状态：
// 等时包错误 = DAC FIFO 溢出 = 主机时钟快于 DAC 时钟。
// 廉价 DAC 在采样率切换后 PLL 可能停在随机偏移速率上（实测可达
// 1000~4500ppm），通过微调 IsoPacketScheduler 的发送速率匹配实际消费速率。
struct SyncDriftState {
    int64_t lastSampleMs = 0;
    int64_t lastIsoErrors = 0;
    int64_t lastStepMs = 0;
    int32_t correctionUnits = 0;
    bool tookPreviousStep = false;
    bool enabled = false;
    // 用户配置开关（reset 后保留）：同步模式下允许主动速率修正。
    // 实测部分同步 DAC（如小尾巴）的 iso 包错误并非速率失配，而是连接/固件抖动，
    // 主动修正反而负反馈恶化（越修越炸）→ 默认关闭，由 Kotlin 设置项控制。
    bool configuredEnabled = false;
    // 累积窗口：错误累积到阈值才修正（突发/间歇错误均能捕捉）
    int64_t accumulatedErrors = 0;
    int64_t accErrorStartMs = 0;
    // 修正有效性：连续多步无改善 → 判定非速率问题，回退并暂停
    int64_t lastStepErrorRateX100 = -1;
    int noImprovementSteps = 0;
    int64_t giveUpUntilMs = 0;
};
...
struct UsbExclusiveHandle {
    ...
    int dupFd = -1;
    // 运行时报告缓存：写路径高频调 writePlayerPcm 持 apiLock 时，
    // nativeRuntimeReport 改用 try_to_lock 并返回缓存，避免与写路径抢锁/GC
    std::mutex reportCacheLock;
    std::string reportCache;
    ...
    std::atomic<int> isoFirstErrorStatus { 0 };
...
void resetSyncDriftState(UsbExclusiveHandle* handle);
void updateSyncDriftRate(UsbExclusiveHandle* handle);
...
bool reconfigureOpenedPlayerPcmOutput(
    ...
    resetSyncDriftState(handle);
    handle->lastTransferBytes.store(0);
    assignNewNativeStreamGeneration(handle);
    if (!parkStreamingAlternate(handle, "reconfigure_ready")) {        if (error != nullptr) {
...
bool startStreamingInternal(
    ...
    handle->isoFirstErrorStatus.store(0);
    ...
    // 切歌/prepare 原地重启动：保留已学到的漂移修正（设备+采样率的属性，
    // 与传输会话无关），只刷新错误基线。清零只在 open/reconfigure（采样率
    // 变化）时发生，避免每首歌重学导致前几秒必出错。
    {
        SyncDriftState& state = handle->syncDrift;
        state.lastSampleMs = 0;
        state.lastStepMs = 0;
        state.lastIsoErrors = handle->isoPacketErrors.load();
        handle->packetScheduler.setRateCorrection(state.correctionUnits);
    }
    {
        std::lock_guard<std::mutex> submitGuard(handle->transferSubmitLock);
...
constexpr int64_t kSyncDriftSampleIntervalMs = 1'000;
constexpr int64_t kSyncDriftStepCooldownMs = 1'500;
// 累积触发阈值：等时错误以"突发"形态出现（一次传输内多包失败、随后间歇），
// 错误累计到阈值才修正；单点抖动（1-3 个错误）在无错误窗口会被衰减掉，
// 不会误修正健康设备。
constexpr int32_t kSyncDriftAccumulateThreshold = 4;
// 无错误窗口后的衰减延时：首次错误后 5s 无新错误才开始衰减，
// 保证慢速残余漂移（0.5-1 错误/秒）仍能累积到阈值。
constexpr int64_t kSyncDriftAccumulateDecayDelayMs = 5'000;
constexpr int32_t kSyncDriftAccumulateDecay = 2;
// 单步最小/最大修正（单位 = 1e-6 帧/interval，@44.1k/8000 时 1 单位≈0.18ppm；
// @384k 时 1 单位≈0.021ppm）。上限 200000 单位 ≈ 3.6% @44.1k / 0.4% @384k。
constexpr int32_t kSyncDriftMinStepUnits = 300;
constexpr int32_t kSyncDriftMaxStepUnits = 20'000;
constexpr int32_t kSyncDriftMaxCorrectionUnits = 200'000;
// 修正无效判定：连续 2 步错误率无显著改善（≥70% 未下降）→ 非速率问题
// （连接质量/端点异常），回退修正并暂停，防止误判把输出速率拖慢 3.6%。
constexpr int32_t kSyncDriftNoImprovementGiveUpSteps = 2;
constexpr int64_t kSyncDriftGiveUpPauseMs = 60'000;

/** 会话/采样率变更后重置漂移补偿状态（同步模式启用，反馈模式禁用） */
void resetSyncDriftState(UsbExclusiveHandle* handle) {
    if (handle == nullptr) {
        return;
    }
    SyncDriftState& state = handle->syncDrift;
    state.enabled = !handle->explicitFeedbackEnabled && state.configuredEnabled;
    state.correctionUnits = 0;
    state.lastSampleMs = 0;
    state.lastStepMs = 0;
    state.lastIsoErrors = handle->isoPacketErrors.load();
    state.accumulatedErrors = 0;
    state.accErrorStartMs = 0;
    state.tookPreviousStep = false;
    state.lastStepErrorRateX100 = -1;
    state.noImprovementSteps = 0;
    state.giveUpUntilMs = 0;
    handle->packetScheduler.setRateCorrection(0);
}

/**
 * 同步模式（explicitFeedbackEnabled == false）下的"累积式"速率修正：
 * - 每秒采样 isoPacketErrors 增量，错误累积到阈值（4）即动作——突发型
 *   错误（一次传输内多包失败）与间歇型错误都能被及时捕捉；
 * - 首次错误后 5s 无新错误则衰减累积量，过滤偶发 USB 单点抖动；
 * - 估算使用实际每包帧数（bytesPerUsbFrame/frameBytes：44.1k=6、384k=48），
 *   避免高采样率下 8 倍低估；
 * - 上一步后错误仍持续（累积 ≥8）时用 1.6 倍追赶，缩短学习期；
 * - 总上限 200000 单位，覆盖廉价 DAC 采样率切换 PLL 失配与长时间热漂移。
 */
void updateSyncDriftRate(UsbExclusiveHandle* handle) {
    if (handle == nullptr || handle->explicitFeedbackEnabled) {
        return;
    ...
    const int64_t now = steadyClockMillis();
    if (!handle->running.load() || !handle->playbackEnabled.load() ||
        handle->focusMuted.load() || !handle->deviceOnline.load() ||
        handle->transportFailed.load() || handle->stopRequested.load() ||
        handle->playerStartupPreroll.framesRemaining() > 0) {
        // 非活跃状态：刷新基线，防止暂停前后拼接出虚假的错误增量
        state.lastIsoErrors = handle->isoPacketErrors.load();
        state.lastSampleMs = now;
        return;
    }
    if (now < state.giveUpUntilMs) {
        // 修正已判定无效（连接质量/端点异常）：暂停修正，仅保持基线
        state.lastIsoErrors = handle->isoPacketErrors.load();
        state.lastSampleMs = now;
        return;
    }
    if (now - state.lastSampleMs < kSyncDriftSampleIntervalMs) {
        return;
    }
    const int64_t errors = handle->isoPacketErrors.load();
    const int64_t errorDelta = errors - state.lastIsoErrors;
    state.lastIsoErrors = errors;
    state.lastSampleMs = now;
    if (errorDelta > 0) {
        if (state.accErrorStartMs == 0) {
            state.accErrorStartMs = now;
        }
        state.accumulatedErrors += errorDelta;
    } else if (state.accumulatedErrors > 0 &&
        now - state.accErrorStartMs >= kSyncDriftAccumulateDecayDelayMs) {
        // 长时间无新错误：衰减累积量（过滤偶发单点错误）
        state.accumulatedErrors = std::max<int64_t>(
            0, state.accumulatedErrors - kSyncDriftAccumulateDecay);
        if (state.accumulatedErrors == 0) {
            state.accErrorStartMs = 0;
        }
    }
    if (state.accumulatedErrors < kSyncDriftAccumulateThreshold) {
        return;
    }
    if (now - state.lastStepMs < kSyncDriftStepCooldownMs) {
        return;
    }
    // 估算失配：每失败包实际帧数（44.1k=6，384k=48）；
    // 1 单位修正 = 1e-6 帧/interval × 8000 interval/s = 0.008 帧/秒。
    const double framesPerPacket = static_cast<double>(
        std::max(1, handle->bytesPerUsbFrame)) / std::max(1, handle->frameBytes);
    const double windowSeconds = std::max(
        1.0, static_cast<double>(now - state.accErrorStartMs) / 1000.0);
    const double lostFramesPerSecond =
        static_cast<double>(state.accumulatedErrors) * framesPerPacket / windowSeconds;
    const double estimatedUnits = lostFramesPerSecond / 0.008;
    const double multiplier =
        (state.tookPreviousStep && state.accumulatedErrors >= 8) ? 1.6 : 1.15;
    const int64_t step = std::clamp<int64_t>(
        static_cast<int64_t>(estimatedUnits * multiplier),
        static_cast<int64_t>(kSyncDriftMinStepUnits),
        static_cast<int64_t>(kSyncDriftMaxStepUnits)
    );
    // 修正有效性判定：错误率（错误/秒）与上一步对比，连续无改善则判定
    // 非速率问题（连接质量/端点异常），回退修正并暂停，避免误判把输出
    // 速率拖慢 3.6%（打满上限）导致音频明显变慢/断续。
    const double currentErrorRate =
        static_cast<double>(state.accumulatedErrors) / windowSeconds;
    if (state.lastStepErrorRateX100 > 0) {
        const double previousErrorRate = state.lastStepErrorRateX100 / 100.0;
        if (currentErrorRate >= previousErrorRate * 0.7) {
            state.noImprovementSteps += 1;
        } else {
            state.noImprovementSteps = 0;
        }
    }
    state.lastStepErrorRateX100 = static_cast<int64_t>(currentErrorRate * 100.0);
    if (state.noImprovementSteps >= kSyncDriftNoImprovementGiveUpSteps) {
        LOGW(
            "sync drift ineffective: %d steps no improvement (rate %.1f/s), "
            "reverting correction (likely connection/endpoint issue)",
            state.noImprovementSteps,
            currentErrorRate
        );
        state.correctionUnits = 0;
        state.giveUpUntilMs = now + kSyncDriftGiveUpPauseMs;
    ...
    // 主机时钟快于 DAC：降低调度速率（仅负方向，溢出才产生可见错误）
    state.correctionUnits = std::max(
        state.correctionUnits - static_cast<int32_t>(step),
        -kSyncDriftMaxCorrectionUnits
    );
    state.lastStepMs = now;
    state.tookPreviousStep = true;
    const int64_t accumulatedBeforeReset = state.accumulatedErrors;
    state.accumulatedErrors = 0;
    state.accErrorStartMs = 0;
    handle->packetScheduler.setRateCorrection(state.correctionUnits);
    LOGW(
        "sync drift correction: errAcc=%lld framesPkt=%.0f est=%.0f units step=%lld "
        "correction=%d units (~%.1f ppm)",
        static_cast<long long>(accumulatedBeforeReset),
        framesPerPacket,
        estimatedUnits,
        static_cast<long long>(step),
        static_cast<int>(state.correctionUnits),
        static_cast<double>(state.correctionUnits) * 0.1814
    );
}
    ...
    try {
        ...
        if (packetsCompleted) {
            ...
            const int totalPacketErrors = handle->isoPacketErrors.fetch_add(failedPacketCount) +
                failedPacketCount;
            int expectedFirstStatus = LIBUSB_TRANSFER_COMPLETED;
            if (firstFailedPacketIndex >= 0) {
                expectedFirstStatus = firstFailedPacketStatus;
            }
            int previousFirstStatus = handle->isoFirstErrorStatus.load();
            while (previousFirstStatus == LIBUSB_TRANSFER_COMPLETED &&
                expectedFirstStatus != LIBUSB_TRANSFER_COMPLETED) {
                if (handle->isoFirstErrorStatus.compare_exchange_weak(
                        previousFirstStatus,
                        expectedFirstStatus
                    )) {
                    break;
...
void eventLoopThread(UsbExclusiveHandle* handle) noexcept {
    try {
        ...
        while (handle->deviceOnline.load() && !handle->stopRequested.load()) {
            // 无反馈（同步）模式等待超时 5ms：缩短 transfer 完成→重提交的最大间隙。
            // 等时传输重提交延迟会造成"无在飞 transfer"窗口 → USB 控制器侧包错误（沙沙声）。
...
Java_com_example_lxmusic_usb_transport_UsbExclusiveNativeBridge_nativeOpen(
    JNIEnv* env,
    jclass /*clazz*/,
    jint fd,
    jint sampleRate,
    ...
    try {
        ...
        resetSyncDriftState(handle.get());
        const int idleAltRc = setStreamingAlternateLocked(
            handle.get(),
            0,
            "open_ready"
...
Java_com_example_lxmusic_usb_transport_UsbExclusiveNativeBridge_nativeStartGeneratedTone(
    JNIEnv* env,
    jclass /*clazz*/,
    jlong handleValue
) {
...
extern "C"
JNIEXPORT jboolean JNICALL
Java_com_example_lxmusic_usb_transport_UsbExclusiveNativeBridge_nativeConfigurePlayerBufferDuration(
    JNIEnv* env,
    jclass /*clazz*/,
    jlong handleValue,
    jint durationMs
...
extern "C"
JNIEXPORT jboolean JNICALL
Java_com_example_lxmusic_usb_transport_UsbExclusiveNativeBridge_nativeConfigurePlayerTransferWindow(
    JNIEnv* env,
    jclass /*clazz*/,
    jlong handleValue,
    jint durationMs
...
extern "C"
JNIEXPORT jboolean JNICALL
Java_com_example_lxmusic_usb_transport_UsbExclusiveNativeBridge_nativePreparePlayerPcm(
    JNIEnv* env,
    jclass /*clazz*/,
    jlong handleValue,
...
Java_com_example_lxmusic_usb_transport_UsbExclusiveNativeBridge_nativeWritePlayerPcm(
    JNIEnv* env,
    jclass /*clazz*/,
    jlong handleValue,
    jobject buffer,
...
extern "C"
JNIEXPORT jboolean JNICALL
Java_com_example_lxmusic_usb_transport_UsbExclusiveNativeBridge_nativeReconfigurePlayerPcmOutput(
    JNIEnv* env,
    jclass /*clazz*/,
    jlong handleValue,
    jint sampleRate,
...
extern "C"
JNIEXPORT jboolean JNICALL
Java_com_example_lxmusic_usb_transport_UsbExclusiveNativeBridge_nativePlayPlayerPcm(
    JNIEnv* env,
    jclass /*clazz*/,
    jlong handleValue
) {
...
extern "C"
JNIEXPORT jboolean JNICALL
Java_com_example_lxmusic_usb_transport_UsbExclusiveNativeBridge_nativeStartPlayerPcm(
    JNIEnv* env,
    jclass clazz,
    jlong handleValue
) {
    return Java_com_example_lxmusic_usb_transport_UsbExclusiveNativeBridge_nativePlayPlayerPcm(
        env,
        clazz,
        handleValue
    );
...
extern "C"
JNIEXPORT jboolean JNICALL
Java_com_example_lxmusic_usb_transport_UsbExclusiveNativeBridge_nativePausePlayerPcm(
    JNIEnv* env,
    jclass /*clazz*/,
    jlong handleValue
) {
...
extern "C"
JNIEXPORT jboolean JNICALL
Java_com_example_lxmusic_usb_transport_UsbExclusiveNativeBridge_nativeFlushPlayerPcm(
    JNIEnv* env,
    jclass /*clazz*/,
    jlong handleValue
) {
...
extern "C"
JNIEXPORT jboolean JNICALL
Java_com_example_lxmusic_usb_transport_UsbExclusiveNativeBridge_nativeSetPlayerVolume(
    JNIEnv* env,
    jclass /*clazz*/,
    jlong handleValue,
    jfloat volume
...
extern "C"
JNIEXPORT jboolean JNICALL
Java_com_example_lxmusic_usb_transport_UsbExclusiveNativeBridge_nativeSetPlayerFocusMuted(
    JNIEnv* env,
    jclass /*clazz*/,
    jlong handleValue,
    jboolean muted
...
extern "C"
JNIEXPORT jboolean JNICALL
Java_com_example_lxmusic_usb_transport_UsbExclusiveNativeBridge_nativeSetSyncDriftEnabled(
    JNIEnv* env,
    jclass /*clazz*/,
    jlong handleValue,
    jboolean enabled
    ...
    SyncDriftState& state = holder->syncDrift;
    const bool shouldEnable = enabled == JNI_TRUE;
    state.configuredEnabled = shouldEnable;
    state.enabled = !holder->explicitFeedbackEnabled && shouldEnable;
    if (!state.enabled) {
        // 关闭：回退已有修正，恢复标称发送速率
        state.correctionUnits = 0;
        state.lastStepMs = 0;
        ...
        state.noImprovementSteps = 0;
        state.giveUpUntilMs = 0;
        holder->packetScheduler.setRateCorrection(0);
        LOGI(
            "sync drift correction disabled for handle=%lld (feedback=%d)",
            static_cast<long long>(handleValue),
            holder->explicitFeedbackEnabled ? 1 : 0
        );
    }
    return JNI_TRUE;
}
...
extern "C"
JNIEXPORT jlong JNICALL
Java_com_example_lxmusic_usb_transport_UsbExclusiveNativeBridge_nativeGetCompletedAudioFrames(
    JNIEnv* env,
    jclass /*clazz*/,
    jlong handleValue
) {
...
extern "C"
JNIEXPORT jlong JNICALL
Java_com_example_lxmusic_usb_transport_UsbExclusiveNativeBridge_nativeGetQueuedPlayerFrames(
    JNIEnv* env,
    jclass /*clazz*/,
    jlong handleValue
) {
...
extern "C"
JNIEXPORT jlong JNICALL
Java_com_example_lxmusic_usb_transport_UsbExclusiveNativeBridge_nativeGetPlayerPcmFreeBytes(
    JNIEnv* env,
    jclass /*clazz*/,
    jlong handleValue
) {
...
Java_com_example_lxmusic_usb_transport_UsbExclusiveNativeBridge_nativeStop(
    JNIEnv* env,
    jclass /*clazz*/,
    jlong handleValue
) {
...
extern "C"
JNIEXPORT void JNICALL
Java_com_example_lxmusic_usb_transport_UsbExclusiveNativeBridge_nativeMarkDeviceDetached(
    JNIEnv* env,
    jclass /*clazz*/,
    jlong handleValue
) {
...
extern "C"
JNIEXPORT void JNICALL
Java_com_example_lxmusic_usb_transport_UsbExclusiveNativeBridge_nativeClose(
    JNIEnv* env,
    jclass /*clazz*/,
    jlong handleValue
) {
...
Java_com_example_lxmusic_usb_transport_UsbExclusiveNativeBridge_nativeRuntimeReport(
    JNIEnv* env,
    jclass /*clazz*/,
    jlong handleValue
) {
    try {
        ...
        // 写路径高频持 apiLock：拿不到锁时返回最近一次成功构建的报告缓存，
        // 避免报告构建阻塞/拖慢 PCM 写入（前台设置页轮询与写路径抢锁 → iso 错误）
        std::unique_lock<std::mutex> apiGuard(holder->apiLock, std::try_to_lock);
        if (!apiGuard.owns_lock()) {
            std::lock_guard<std::mutex> cacheGuard(holder->reportCacheLock);
            if (holder->reportCache.empty()) {
                return env->NewStringUTF("reportVersion=2 busy");
            }
            return env->NewStringUTF(holder->reportCache.c_str());
        }
        ...
        const std::string report =
            ...
            " driftCorrection=" + std::to_string(holder->syncDrift.correctionUnits) +
            "u driftEnabled=" + std::string(holder->syncDrift.enabled ? "true" : "false") +
            ...
            " isoFirstErrorStatus=" + std::to_string(holder->isoFirstErrorStatus.load()) +
...
extern "C"
JNIEXPORT jstring JNICALL
Java_com_example_lxmusic_usb_transport_UsbExclusiveNativeBridge_nativeAcknowledgeRecoveryAction(
    JNIEnv* env,
    jclass /*clazz*/,
    jlong handleValue,
...
extern "C"
JNIEXPORT jstring JNICALL
Java_com_example_lxmusic_usb_transport_UsbExclusiveNativeBridge_nativeLastOpenError(
    JNIEnv* env,
    jclass /*clazz*/
) {
    try {
