 ...
 *
 * File: moe.ouom.neriplayer.core.player.usb.transport/UsbExclusiveNativeBridge (adapted for LxMusic)
 */
...
/** JNI 桥：与 liblxmusic_usb.so（libusb + UAC1/UAC2 native 栈）通信 */
internal object UsbExclusiveNativeBridge {
    private const val TAG = "LxUsbNative"
    ...
    private inline fun callNativeBoolean(
        ...
        if (!succeeded) {
            UsbExclusiveLog.e(TAG, "$operation returned false (${context()})")
        }
        return succeeded
    }
    ...
    private fun logNativeFailure(operation: String, context: String?, error: Throwable) {
        ...
        UsbExclusiveLog.e(TAG, "$operation threw$contextSuffix: $message")
    }
    ...
    fun ensureLoaded(): Boolean {
        ...
        synchronized(this) {
            ...
            loadAttempted = true
            return try {
                System.loadLibrary("lxmusic_usb")
                libraryReady = true
                true
            } catch (error: Exception) {
    ...
    fun setSyncDriftEnabled(handle: Long, enabled: Boolean): Boolean {
        if (handle == 0L || !ensureLoaded()) return false
        return callNativeBoolean(
            operation = "nativeSetSyncDriftEnabled",
            context = { "handle=$handle enabled=$enabled" }
        ) {
            nativeSetSyncDriftEnabled(handle, enabled)
        }
    }
    ...
    fun stop(handle: Long) {
        ...
        callNative(operation = "nativeStop", context = { "handle=$handle" }) {
            nativeStop(handle)
        }
    }
    ...
    fun markDeviceDetached(handle: Long) {
        if (handle == 0L || !ensureLoaded()) return
        callNative(operation = "nativeMarkDeviceDetached", context = { "handle=$handle" }) {
    ...
    fun close(handle: Long) {
        if (handle == 0L || !ensureLoaded()) return
        callNative(operation = "nativeClose", context = { "handle=$handle" }) {
            nativeClose(handle)
        }
    }
    ...
    /**
     * 确认 Kotlin 侧已受理原生建议的终端恢复动作（FreshOpen / StopPreserveIntent）。
     * 返回 C++ 的 ack 状态名：Acked / AlreadyAcked / GenerationMismatch / HandleClosing / NoPending。
     * actionGeneration 与 actionId 取自运行时报告（actionGeneration / actionId 字段）。
     */
    fun acknowledgeRecoveryAction(
        ...
        return callNative(
            ...
            context = { "handle=$handle actionGeneration=$actionGeneration actionId=$actionId" }
        ) {
    ...
    private external fun nativeSetSyncDriftEnabled(handle: Long, enabled: Boolean): Boolean
